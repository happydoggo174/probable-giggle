...
solution_route.get("/list",async({query})=>{
    return await get_connection(async(db)=>await db`select solution_id,title,account.username 
    from solution 
    inner join account 
...
.get("/detail",async({query})=>{
    return await get_connection(async(db)=>{
        const r=await db`select title,author_id,account.username,account.profile,content,is_plain 
        from solution 
        inner join account
        on solution.author_id=account.uid 
...
query:z.object({
    ...
    await get_connection(async(db)=>{
        try{
            await db`insert into solution(problem_id,author_id,title,content,is_plain) 
            values(${body.problem_id},${user.user_id},${body.title},${body.content},${body.plain_content})`
        }catch(e){
            console.log(e);
